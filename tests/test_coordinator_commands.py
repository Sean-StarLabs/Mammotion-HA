...
async def test_send_and_wait_returns_protocol_response() -> None:
    ...
    device = SimpleNamespace(online=True)
    coordinator.manager.get_device_by_name.return_value = device
    coordinator.manager.mower.return_value = SimpleNamespace(has_usable_transport=True)
...
@pytest.mark.asyncio
async def test_usable_transport_can_send_with_disconnected_report_state() -> None:
    """A reconnectable BLE transport is not blocked by stale telemetry state."""
    response = object()
    coordinator = object.__new__(_TestCoordinator)
    coordinator.manager = MagicMock()
    coordinator.manager.get_device_by_name.return_value = SimpleNamespace(online=False)
    coordinator.manager.mower.return_value = SimpleNamespace(
        has_usable_transport=True,
