...
def _assert_cancel_calls(entity: MammotionLawnMowerEntity) -> None:
    """Assert each cancellation step waits for its own action ACK."""
    requests = entity.coordinator.async_send_and_wait.await_args_list
    assert [request.args for request in requests] == [
        ("pause_execute_task", "todev_taskctrl_ack"),
        ("cancel_job", "todev_taskctrl_ack"),
    ]
    for request, action in zip(requests, (2, 4), strict=True):
        predicate = request.kwargs["response_predicate"]
        assert predicate(_ack(action))
        assert not predicate(_ack(99))
