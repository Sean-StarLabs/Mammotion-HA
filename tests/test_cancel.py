...
async def test_cancel_rejects_failed_acknowledgement() -> None:
    ...
    entity.coordinator.async_send_and_wait.assert_awaited_once_with(
        ...
        "todev_taskctrl_ack",
        preempt_reads=True,
        response_predicate=entity.coordinator.async_send_and_wait.await_args.kwargs[
            "response_predicate"
        ],
    )
    predicate = entity.coordinator.async_send_and_wait.await_args.kwargs[
        "response_predicate"
    ]
    assert not predicate(_ack(2))
    assert predicate(_ack(4))
