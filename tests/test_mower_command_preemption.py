...
@pytest.mark.asyncio
async def test_cancelled_service_retains_safety_controls_after_dispatch() -> None:
    """An accepted start remains controllable until telemetry resolves its state."""
    entity = _new_entity()
    dispatched = asyncio.Event()
    release = asyncio.Event()

    async def dispatch_then_wait(**_kwargs: object) -> None:
        entity._start_dispatching = True
        dispatched.set()
        await release.wait()
        entity._start_dispatching = False
    ...
    entity._async_start_transaction = dispatch_then_wait  # type: ignore[method-assign]
    task = asyncio.create_task(entity.async_start_mowing())
    ...
    with patch.object(
        ...
        return_value=SimpleNamespace(can_start=False, can_pause=False, can_dock=False),
    ):
        assert entity.supported_features & lawn_mower.LawnMowerEntityFeature.PAUSE
        assert entity.supported_features & lawn_mower.LawnMowerEntityFeature.DOCK
...
@pytest.mark.asyncio
async def test_pending_start_feature_changes_are_published() -> None:
    """HA is notified when safety controls appear and disappear."""
    entity = _new_entity()
    ...
    async def start_transaction(**_kwargs: object) -> None:
        started.set()
        await finish.wait()

    entity._async_start_transaction = start_transaction  # type: ignore[method-assign]
    task = asyncio.create_task(entity.async_start_mowing())
    await started.wait()

    assert entity._start_cancel_events
    entity.async_write_ha_state.assert_called_once_with()

    finish.set()
    await task

    assert not entity._start_cancel_events
...
async def test_pause_supersedes_an_accepted_unreported_start() -> None:
    ...
    entity._async_task_control.assert_awaited_once_with(
        "pause_execute_task",
        action=2,
        expected_modes=lawn_mower.PAUSED_MODES,
        translation_key="pause_failed",
        timeout=lawn_mower.START_PREEMPT_CONFIRM_TIMEOUT,
    )
