...
def test_in_place_native_geometry_change_updates_signature() -> None:
    """Released clients that mutate route containers still trigger storage."""
    map_data = _map()
    map_data.dynamics_line = [{"x": 1, "y": 2}]
    before = native_trail_signature(map_data)

    map_data.dynamics_line[0]["x"] = 3

    assert native_trail_signature(map_data) != before


def test_dynamics_fingerprint_stays_fixed_width() -> None:
    """Long live routes do not grow the rolling fingerprint integer."""
    map_data = _map()
    map_data.dynamics_line = [{"x": index, "y": -index} for index in range(10_000)]

    _, fingerprint = native_trail_signature(map_data)[1]

    assert fingerprint.bit_length() <= 64
