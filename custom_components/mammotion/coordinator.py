...
class MammotionBaseUpdateCoordinator[DataT](DataUpdateCoordinator[DataT]):  # type: ignore[misc]
    ...
    def __init__(
        ...
        self.operation_settings_lock = asyncio.Lock()
    ...
    async def async_ensure_fresh_report_data(self) -> bool:
        ...
        if ensure_fresh is None or not self.command_ready:
            return False
        return await ensure_fresh(
            self.device_name,
            max_age_s=5.0,
            timeout=10.0,
        )
    ...
    def report_data_token(self) -> int:
        ...
        report_data_token = getattr(self.manager, "report_data_token", None)
        return report_data_token(self.device_name) if report_data_token else 0

    async def async_wait_for_report_data(self, *, since: int, timeout: float) -> bool:
        """Wait for telemetry newer than *since*."""
        wait_for_report_data = getattr(self.manager, "wait_for_report_data", None)
        if wait_for_report_data is None:
            return False
        return await wait_for_report_data(
            self.device_name,
            since=since,
            timeout=timeout,
        )
    ...
    async def async_modify_plan_if_mowing(self) -> None:
        ...
        if _mdata.report_data.dev.sys_status != WorkMode.MODE_WORKING:
            return
        result = await self.async_modify_plan_route(
