...
def migrate_area_hashes(
    settings: OperationSettings, migrated_hashes: dict[int, int]
) -> bool:
    """Replace selected area hashes after the mower rebuilds named areas."""
    migrated = list(
        dict.fromkeys(migrated_hashes.get(area, area) for area in settings.areas)
    )
    if migrated == settings.areas:
        return False
    settings.areas = migrated
    return True
