...
PAUSED_MODES = {WorkMode.MODE_PAUSE, WorkMode.MODE_CHARGING_PAUSE}
...
class MammotionLawnMowerEntity(MammotionBaseEntity, LawnMowerEntity):  # type: ignore[misc]
    ...
    def _preempt_pending_start(self) -> _StartPreemption:
        ...
        preemption = _StartPreemption(
            pending=bool(self._start_cancel_events) or self._start_outcome_uncertain,
            may_be_active=(
                self._start_dispatching
                or self._start_dispatched
                or self._start_outcome_uncertain
            ),
        )
    ...
    def supported_features(self) -> LawnMowerEntityFeature:
        ...
        if self._start_cancel_events or self._start_outcome_uncertain:
    ...
    async def _async_task_control(
        ...
        *,
        ...
        def matches_action(response: object) -> bool:
            try:
                field, ack = betterproto2.which_one_of(response.nav, "SubNavMsg")
            except (AttributeError, TypeError, ValueError):
                return False
            return field == "todev_taskctrl_ack" and int(ack.action) == action
        ...
        try:
            response = await self.coordinator.async_send_and_wait(
                ...
                "todev_taskctrl_ack",
                preempt_reads=True,
                response_predicate=matches_action,
            )
        finally:
            if is_start_command:
                self._start_dispatching = False
