...
def unambiguous_area_names(area_names_by_hash: dict[int, str]) -> set[str]:
    """Return normalized names that identify exactly one current area."""
    counts = Counter(name.casefold() for name in area_names_by_hash.values())
...
def reported_area_name_hashes(mower_map: Any) -> set[int]:
    """Return every area covered by the latest name-list response."""
    reported: set[int] = set()
    for area in getattr(mower_map, "area_name", ()):
        try:
            reported.add(int(area.hash))
        except (TypeError, ValueError):
            continue
    return reported


def authoritative_area_hashes(mower_map: Any) -> set[int] | None:
    """Return complete area membership from the newest available protocol."""
    manifest = getattr(mower_map, "area_manifest_hashes", None)
    if manifest is not None:
        return set(manifest)
    if area_names_loaded(mower_map):
        return reported_area_name_hashes(mower_map)
    return None


def registry_area_name(entry: Any) -> str | None:
    """Return the device area name retained by an entity registry entry."""
    placeholders = getattr(entry, "translation_placeholders", None) or {}
    if name := str(placeholders.get("name", "")).strip():
        return name

    original_name = str(getattr(entry, "original_name", "") or "").strip()
    if original_name.lower().startswith("area "):
        return original_name[5:].strip() or None
